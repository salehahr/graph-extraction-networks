...
from tools.image import classifier_preview, generate_outputs
from tools.plots import plot_classifier_images, plot_generated_images, plot_sample
...
class TestDataAugmentation(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        img_masked = img_to_array(
            load_img(img_masked_fp, grayscale=True), dtype=np.uint8
        )
        img_skeletonised = img_to_array(
            load_img(img_skeletonised_fp, grayscale=True), dtype=np.uint8
        )
        ...
        cls.plot_title = os.path.relpath(
            img_cropped_fp.replace("cropped/", ""), start=base_path
        )
        sample_images = {
            "cropped": img_cropped,
            "masked": img_masked,
    ...
    def test_transformed_imgs_and_mask(self):
        ...
        in_iter = image_datagen.flow(self.samples_skeletonised, batch_size=1, seed=seed)
        out_node_pos_iter = mask_datagen.flow(
            self.samples_node_pos, batch_size=1, seed=seed
        )
        out_degrees_iter = mask_datagen.flow(
            self.samples_degrees, batch_size=1, seed=seed
        )
        out_node_types_iter = mask_datagen.flow(
            self.samples_node_types, batch_size=1, seed=seed
        )

        classifier_iterators = {
            "node_pos": out_node_pos_iter,
